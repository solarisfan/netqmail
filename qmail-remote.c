...
/* remember last couple of bytes sent. debugging */
static char last_step[256];
...
int safewrite(fd,buf,len) int fd; char *buf; int len;
{
  ...
  *last_step = '\0';
  ...
  if (r <= 0) {
	int nbytes = 80;
	if (len < 80) nbytes = len;
	memcpy(last_step, buf, nbytes);
	last_step[nbytes] = '\0';
  }
  if (r <= 0) dropped();
  return r;
...
char inbuf[1024];
substdio ssin = SUBSTDIO_FDBUF((int(*)())read,0,inbuf,sizeof inbuf);
char smtptobuf[1024];
